/*---------------------------------------------------------------------
  $Header: /WWW/sqlutil/beta_lockinfo.sql 28    22-12-30 20:20 Sommar $

  This SP lists locking information for all active processes, that is,
  processes that have a running request, are holding locks or have an
  open transaction. Information about all locked objects are included,
  as well as the last command sent from the client and the currently
  running statement. The procedure also displays the blocking chain
  for blocked processes.

  Note that locks and processes are read independenty from the DMVs, and
  may not be fully in sync. Likewise, object names and DBCC INPUTBUFFER
  are run later.

  This version of the procedure is for SQL 2016 and later as well as Azure 
  Managed Instance. It does not support Azure SQL Database. There are
  other versions SQL 2012/14, SQL 2008 and SQL 2005.
  ...
  $History: beta_lockinfo.sql $
 * 
 * *****************  Version 28  *****************
 * User: Sommar       Date: 22-12-30   Time: 20:20
 * Updated in $/WWW/sqlutil
 * 1) Fixed the version check, so that the procedure is accepted on Azure
 * SQL Managed Instance.
 * 2) On SQL 2022, the minimum permisson is VIEW SERVER PERFORMANCE STATE.
 * 3) Fine-tuned the filtering of locks so only shared locks on database
 * with owner SHARED_TRANSACTION_WORKSPACE are considered uninteresting.
 * 4) When translating object names, check for single-user databases, so
 * we don't try to access them if someelse is using them.
 * 
 * *****************  Version 27  *****************
 * User: Sommar       Date: 22-03-22   Time: 21:06
 * Updated in $/WWW/sqlutil
 * Corrected typos.
 * 
 * *****************  Version 26  *****************
 * User: Sommar       Date: 20-09-04   Time: 11:13
 * Updated in $/WWW/sqlutil
 * Ignore any error when setting job name.
 *
 * *****************  Version 25  *****************
 * User: Sommar       Date: 20-09-03   Time: 22:54
 * Updated in $/WWW/sqlutil
 * Replace the job guid in the appl column with the name of the job.
 *
 * *****************  Version 24  *****************
 * User: Sommar       Date: 20-03-08   Time: 23:15
 * Updated in $/WWW/sqlutil
 * 1) Show the start times for the individual requests for a MARS-enabled
 * process.
 * 2) When stmt_start = stmt_end, display the first 1000 characters from
 * sys.dm_exec_sql_text. This is helpful for xp_cmdshell and
 * sp_execute_external_script.
 * 3) Fixed formatting when all we have to display for an object name is a
 * 64-bit number.
 *
 * *****************  Version 23  *****************
 * User: Sommar       Date: 19-06-04   Time: 21:42
 * Updated in $/WWW/sqlutil
 * In the format db.<object_id> asterisks was sometimes displayed because
...
IF @edition <> 8   -- That is, Azure Managed Instance.
BEGIN
...
CREATE OR ALTER PROCEDURE beta_lockinfo @allprocesses  tinyint = 0,
                                        @textmode      bit     = 0,
...
DECLARE @locks_takeone TABLE (
   ...
   activelock AS CASE WHEN rsc_type = 'DATABASE' AND
                           req_owner_type = 'SHARED_TRANSACTION_WORKSPACE' AND
                           req_status = 'GRANT'  AND
                           req_mode = 'S'
                      THEN convert(bit, 0)
                      ELSE convert(bit, 1)
                 END,
...
DECLARE @now            datetime2(3) = sysdatetime(),
        ...
        @stmt_start     int,
        @stmt_end       int,
        @minimumperm    varchar(100),
        @version        int = convert(int, serverproperty('ProductMajorVersion')),
        @edition        int = convert(int, serverproperty('EngineEdition'))
...
SET @minimumperm = IIF(@version >= 16 OR @edition = 8,
                       'VIEW SERVER PERFORMANCE STATE', 'VIEW SERVER STATE')

IF NOT EXISTS (SELECT *
               FROM   sys.fn_my_permissions(NULL, NULL)
               WHERE  permission_name = @minimumperm)
BEGIN
   RAISERROR('You need to have the permission %s to run this procedure', 16, 1, @minimumperm)
   RETURN
END
...
INSERT @procs(session_id, task_address,
   ...
   SELECT es.session_id, coalesce(ot.task_address, 0x),
          ...
          es.login_time, coalesce(er.start_time, es.last_request_start_time),
          CASE WHEN abs(datediff(YEAR, coalesce(er.start_time, es.last_request_start_time), @now)) > 60
               THEN NULL
               ELSE datediff_big(MS, coalesce(er.start_time, es.last_request_start_time),  @now) / 1000.000
          END,
...
------------------------------------------------------------------------
-- Replace binary guids with jobs names in program_name. Since this is
-- not essential, ignore any error.
------------------------------------------------------------------------
IF @debug = 1
BEGIN
   SELECT @ms = datediff(ms, @now, sysdatetime())
   RAISERROR ('Getting names of Agent jobs.', 0, 1, @ms) WITH NOWAIT
END

BEGIN TRY
   UPDATE @procs
   SET    program_name = 'SQL Agent - TSQL JobStep (Job ' + quotename(j.name) +
                          substring(p.program_name, 64, len(p.program_name))
   FROM   @procs p
   JOIN   msdb.dbo.sysjobs j ON j.job_id = CAST(convert(binary(16), substring(p.program_name, 30, 34), 1)
                                                AS uniqueidentifier)
...
BEGIN TRY
   ...
   SET    curdbid      = est.dbid,
          ...
          CASE WHEN est.encrypted = 1
               ...
               WHEN p.stmt_start = p.stmt_end
                  THEN substring(est.text, 1, 1000)
               WHEN p.stmt_start >= 0
...
WHILE 1 = 1
BEGIN
   FETCH C2 INTO @dbid, @dbidstr, @dbname
   IF @@fetch_status <> 0
      BREAK

   -- Check that the database is not in single-user mode and used by someone else.
   -- If so, let's make it quick.
   IF (SELECT user_access_desc FROM sys.databases WHERE database_id = @dbid) = 'SINGLE_USER' AND
      NOT EXISTS (SELECT * FROM @locks_final WHERE session_id = @@spid AND database_id = @dbid)
   BEGIN
      UPDATE #objects
      SET    object_name = 'Database is in single-user mode.'
   ...
   -- This is particularly important when we only have VIEW SERVER PERFORMANCE STATE,
