...
SELECT 
     ...
     , IIF([si].[has_filter] = 1, CONCAT('WHERE ', [si].[filter_definition]), '')   AS [FilteredDefinition]
...
OUTER APPLY 
(
    SELECT DISTINCT [kc].[object_id] FROM sys.key_constraints AS [kc] WHERE [kc].[name] = [si].[name]
) AS [kc]
/*
