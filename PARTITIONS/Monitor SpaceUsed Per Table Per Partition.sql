...
FROM 
    ...
    INNER JOIN (                                                                                                                                   
                    SELECT MIN([SystemUpdatedDt]) AS [MinVaue], MAX([SystemUpdatedDt]) AS [MaxVaue], 'YourTableName_Current' AS [Table_Name]    FROM dbo.YourTableName_Current    (NOLOCK)
                    UNION                                                                                                                                     
                    SELECT MIN([SystemUpdatedDt]) AS [MinVaue], MAX([SystemUpdatedDt]) AS [MaxVaue], 'YourTableName_Archive' AS [Table_Name]    FROM dbo.YourTableName_Archive    (NOLOCK)
                    UNION                                                                                                                                     
                    SELECT MIN([SystemUpdatedDt]) AS [MinVaue], MAX([SystemUpdatedDt]) AS [MaxVaue], 'YourTableName_RecycleBin' AS [Table_Name] FROM dbo.YourTableName_Archive    (NOLOCK)
               
               )    tbl
               ON   tbl.[Table_Name] = st.name
--WHERE au.UsedMB > 0
)
SELECT 
        *
...
FROM cte 
WHERE 
        cte.TableName IN ('YourTableName')
AND     cte.UsedMB > 0
AND     cte.Rows > 0
