...
DECLARE @SchemaNames                         NVARCHAR(MAX) = N'Production,'	--N'dbo,'		  --'Sales;Production;HumanResources;Person;'
      , @TableNames                          NVARCHAR(MAX) = N'Product,'	--N'CoverLegend,' --'SpecialOfferProduct;Product;Employee;BusinessEntity;Person;'
      ...
      , @DropReferencesToEmptyTables		 BIT		   = 0   /* 0 = drop and re-create only FKs referencing non-empty [#SelectedTables], 
																	1 = drop and re-create all FKs referencing [#SelectedTables] - may impact performance! */
...
SET @Params = N'@_ErrorMessage NVARCHAR(4000) OUTPUT'
...
--DELETE FROM [#SelectedTables] WHERE [TableName] = 'CoverLegend'
...
BEGIN
	...
	--WHERE			[st].[RowCountBefore] > CASE WHEN @DropReferencesToEmptyTables = 1 THEN -1 ELSE 0 END
	)
	...
	--AND	   [st].[RowCountBefore] > CASE WHEN @DropReferencesToEmptyTables = 1 THEN -1 ELSE 0 END
...
/* ########################################################## RECREATING AND RE-ENABLING: #################################################### */
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @SqlRecreateCDC = (CONCAT(
			 ' BEGIN TRY ',
			 ' EXECUTE sys.sp_cdc_enable_table
			   @source_schema	= ', COALESCE(QUOTENAME(@CDC_source_schema  , ''''), 'NULL'), '
			 , @source_name		= ', COALESCE(QUOTENAME(@CDC_source_name	, ''''), 'NULL'), '
			 , @role_name		= ', COALESCE(QUOTENAME(@CDC_role_name		, ''''), 'NULL'), '
			 , @filegroup_name	= ', COALESCE(QUOTENAME(@CDC_filegroup_name	, ''''), 'NULL'),
			 ' END TRY ',
			 ' BEGIN CATCH ',
			...
			PRINT(@SqlRecreateCDC)
			EXEC @Return = sys.sp_executesql @SqlRecreateCDC, @Params, @_ErrorMessage = @ErrorMessage OUTPUT;
			IF @Return <> 0 OR LEN(@ErrorMessage) > 0
			    BEGIN
			...
			*/
			...
			BEGIN
				ROLLBACK TRANSACTION
				SET @ErrorMessage = CONCAT('Rolling back transaction - Error while executing sys.sp_cdc_enable_table with parameters @source_schema: '
											, @CDC_source_schema, ' @source_name: ', @CDC_source_name, ' @role_name: ', @CDC_role_name, ' @filegroup_name: ', @CDC_filegroup_name);
				GOTO ERROR																								  		
			END
			ELSE 
			*/
		...
		BEGIN
			--ROLLBACK TRANSACTION
			SET @ErrorMessage = CONCAT('Rolling back transaction - Number of CDC-Re-Enabled Tables: ', @CountCDCReenabled
									, ' does not match the Number of CDC-Enabled Tables Disabled: ', @CountCDCDisabled);              
			--GOTO ERROR
			PRINT(@ErrorMessage)
		END
	...
	BEGIN
		...
		BEGIN
			...
			IF (@SqlXtndProperties IS NOT NULL) EXEC sys.sp_executesql @SqlXtndProperties;
		...
		BEGIN
			--ROLLBACK TRANSACTION
			SET @ErrorMessage = CONCAT('Rolling back transaction - Number of Schema-Bound Views Recreated: ', @CountSchBvRecreated
									, ' does not match the Number of Schema-Bound Views dropped: ', @CountSchBvDropped);              
			--GOTO ERROR
			PRINT(@ErrorMessage)
		END
		ELSE
		BEGIN			
			PRINT(CONCAT('Successfully Recreated: ', COALESCE(@CountSchBvRecreated, 0), ' Schema-Bound Views (matches the number of Schema-Bound Views Dropped).'))		
		END
	...
	BEGIN
		...
		BEGIN
			--ROLLBACK TRANSACTION
			SET @ErrorMessage = CONCAT('Rolling back transaction - Number of FK Constraints Re-Created: ', @CountFKRecreated
									, ' does not match the Number of FK Constraints dropped: ', @CountFKDropped);              
			--GOTO ERROR
			PRINT(@ErrorMessage)
		END
...
--		--SELECT * FROM [#SelectedTables] WHERE [IsCDCEnabled] = 1 ORDER BY [RowCountBefore] DESC
--		SELECT * FROM [#ForeignKeyConstraintDefinitions] ORDER BY [Id]
