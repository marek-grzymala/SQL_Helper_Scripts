...
(
  ...
  /*, INDEX [#IX_TableRowCounts_GtZ]([RowCount]) WHERE [RowCount] > 0  -- does not help, selects use scan anyway */
)
...
BEGIN
	...
	PRINT('---------------------------------------------------- GETTING TABLE ROWCOUNTS [#SelectedObjectList]: ----------------------------------------------------')

	SELECT @LineId = MIN([Id]), @LineIdMax = MAX([Id]) FROM [#SelectedObjectList];
	WHILE (@LineId <= @LineIdMax) 
	BEGIN
		SELECT	
			 @SqlTableCounts = STRING_AGG(CONCAT('SELECT '
	...
	UPDATE [sol]
	SET    [sol].[RowCount] = [trc].[RowCount]
	FROM   [#SelectedObjectList] AS [sol]
	JOIN   [#TableRowCounts] AS [trc] ON [trc].[ObjectID] = [sol].[ObjectID]
	
	SELECT * FROM [#SelectedObjectList] WHERE [RowCount] > 0 ORDER BY [RowCount] DESC
	...
	WHERE			[sol].[RowCount] > 0 /* drop only constraints referring to non-empty target tables */
	)
	INSERT INTO  [#ForeignKeyConstraintDefinitions](
	             [ForeignKeyId]              
