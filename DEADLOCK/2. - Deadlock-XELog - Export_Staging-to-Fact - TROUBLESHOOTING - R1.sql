	...
	BEGIN
		...
		*/
		...
		SELECT @rn_per_timestamp = MIN(rn_per_timestamp)
		     , @max_rn_per_timestamp = MAX(rn_per_timestamp)
		FROM   [#DeadlockIds]
		...
		WHILE @@FETCH_STATUS = 0 
		BEGIN
				WHILE (@rn_per_timestamp <= @max_rn_per_timestamp)
				BEGIN
					...
					ON  [did].[transaction_id]		 = @transaction_id
					...
					UPDATE [#DeadlockIds] SET [matched_with_xml] = 1 WHERE [rn_per_timestamp] = @rn_per_timestamp;
